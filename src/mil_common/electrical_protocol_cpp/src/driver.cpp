...
void SerialDevice::close()
{
  ...
  {
    Packet packet = std::move(writeQueue_.front());
    writeQueue_.pop();
    onWrite(packet, EINTR);
  }

  for (auto& pair : readQueues_)
  {
  ...
  while (1)
  {
    ...
    else if (state == ReadState::DATA)
    {
      ...
      if (!findPacket)
      {
        ...
        {
          bytesRead += ret;
          ret = ::read(device->serialFd_, buffer + (bytesRead - Packet::HEADER_LEN) % bufferLen,
                       std::min(bufferLen, bytesToRead - bytesRead));
        }
