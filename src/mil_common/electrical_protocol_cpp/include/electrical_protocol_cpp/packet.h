...
class Packet
{
  public:
    ...
    explicit Packet(std::pair<uint8_t, uint8_t> const& id) noexcept : id_(id)
    {
    }
    ...
    inline uint8_t const& operator[](size_t index) const
    {
        if (index + HEADER_LEN + TRAILER_LEN >= data_.size())
            throw std::out_of_range("No enough data");
    ...
    struct IdHash
    {
        size_t operator()(std::pair<uint8_t, uint8_t> const& p) const
        {
            return static_cast<size_t>((static_cast<uint16_t>(p.first) << 8) + p.second);
        }
    };

    inline std::pair<uint8_t, uint8_t> const& id() const noexcept
    {
        return id_;
    }
    ...
    auto unpack(Fmt) const
    {
        ...
        if (dataLen + HEADER_LEN + TRAILER_LEN > data_.size())
            throw std::out_of_range("No enough data: expect " + std::to_string(dataLen) + " has " +
                                    std::to_string(size()));
  ...
  private:
    ...
    std::pair<uint8_t, uint8_t> const id_;
    ...
    constexpr void pack_(std::index_sequence<Items...>, Args&&... args)
    {
        ...
        using Types =
            std::tuple<typename pystruct::RepresentedType<decltype(formatMode), formats[Items].formatChar>...>;

        // Convert args to a tuple of the represented types
        Types t = std::make_tuple(
            pystruct::internal::convert<std::tuple_element_t<Items, Types>>(std::forward<Args>(args))...);

        constexpr size_t offsets[] = { pystruct::getBinaryOffset<Items>(Fmt{})... };
        int _[] = { 0,
                    pystruct::internal::packElement(reinterpret_cast<char*>(data_.data()) + HEADER_LEN + offsets[Items],
                                                    formatMode.isBigEndian(), formats[Items], std::get<Items>(t))... };
    ...
    constexpr auto unpack_(std::index_sequence<Items...>) const
    {
        ...
        auto unpacked = std::make_tuple(pystruct::unpackElement<Items, std::tuple_element_t<Items, Types>>(
            reinterpret_cast<char const*>(data_.data()) + HEADER_LEN + offsets[Items], formats[Items].size,
