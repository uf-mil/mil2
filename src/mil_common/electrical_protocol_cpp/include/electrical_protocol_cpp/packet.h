...
public:
  ...
  auto unpack(Fmt) const
  {
    ...
    if (dataLen + HEADER_LEN + TRAILER_LEN > data_.size())
      throw std::out_of_range("No enough data: expect " + std::to_string(dataLen) + " has " + std::to_string(size()));
...
private:
  ...
  constexpr void pack_(std::index_sequence<Items...>, Args&&... args)
  {
    ...
    Types t =
        std::make_tuple(pystruct::internal::convert<std::tuple_element_t<Items, Types>>(std::forward<Args>(args))...);

    constexpr size_t offsets[] = { pystruct::getBinaryOffset<Items>(Fmt{})... };
    int _[] = { 0, pystruct::internal::packElement(reinterpret_cast<char*>(data_.data()) + HEADER_LEN + offsets[Items],
