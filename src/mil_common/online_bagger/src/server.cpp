...
class Server : public rclcpp::Node
{
  public:
    Server()
      : rclcpp::Node("online_bagger_server")
      , work_guard(boost::asio::make_work_guard(context))
      , work_thread([this] { context.run(); })
    {
        get_params();

        topics_service = create_service<mil_msgs::srv::BagTopics>(
            TOPIC_SERVICE_NAME,
            std::bind(&Server::handle_topics_req, this, std::placeholders::_1, std::placeholders::_2));

        _action_server = rclcpp_action::create_server<mil_msgs::action::BagOnline>(
            this, BAG_ACTION_NAME, std::bind(&Server::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
        ...
        {
            RCLCPP_INFO(get_logger(), "Subscribed to %ld of %ld topics", successful_subscription_count,
                        subscriber_list.size());
        }
        else
        {
            RCLCPP_INFO(get_logger(), "Subscribed to %ld of %ld topics, will try again every %.2f seconds",
                        successful_subscription_count, subscriber_list.size(), resubscribe_period);
            resubscriber =
                create_wall_timer(std::chrono::milliseconds(static_cast<int>(resubscribe_period * 1000)),
                                  [this]
                                  {
                                      if ((successful_subscription_count == subscriber_list.size()) && resubscriber)
                                      {
                                          ...
                                          {
                                              RCLCPP_INFO(get_logger(), "All topics are subscribed! Shutting down "
                                                                        "resubscriber");
                                          }
  ...
  private:
    ...
    std::unordered_map<std::string, std::deque<std::pair<rclcpp::Time, std::shared_ptr<rclcpp::SerializedMessage>>>>
        topic_messages;
    ...
    {
        for (auto const& [topic, info] : subscriber_list)
        {
            auto const& [time, subscribed] = info;
    ...
    handle_goal([[maybe_unused]] rclcpp_action::GoalUUID const& uuid,
                [[maybe_unused]] std::shared_ptr<mil_msgs::action::BagOnline::Goal const> goal)
    {
        if (!streaming)
        {
    ...
    rclcpp_action::CancelResponse handle_cancel([[maybe_unused]] std::shared_ptr<BagOnlineGoalHandle> const goal_handle)
    {
        return rclcpp_action::CancelResponse::REJECT;
    }

    float get_topic_duration(std::string const& topic)
    {
        if (topic_messages[topic].size() == 0)
            return 0.0f;

        return (topic_messages[topic].back().first - topic_messages[topic].front().first).nanoseconds() / 1e9f;
    }

    size_t get_time_index(std::string const& topic, float requested_seconds)
    {
    ...
    {
        context.post([this, goal_handle] { start_bagging(goal_handle); });
    }

    void bagger_callback(std::shared_ptr<rclcpp::SerializedMessage> msg, std::string const& topic)
    {
        if (!streaming)
            return;
        ...
        {
            RCLCPP_DEBUG(get_logger(), "%s has %ld messages spanning %.2f seconds.", topic.c_str(),
                         topic_messages[topic].size(), time_diff);
        }
    ...
    std::filesystem::path get_bag_name(std::filesystem::path const& filename)
    {
        std::filesystem::path default_dir = dir;
        std::string date = boost::gregorian::to_simple_string(boost::gregorian::day_clock::local_day());
        std::string time =
            boost::posix_time::to_simple_string(boost::posix_time::second_clock::local_time().time_of_day());
        if (dated_folder)
        {
            default_dir /= date;
        }
    ...
    void start_bagging(std::shared_ptr<BagOnlineGoalHandle> goal_handle)
    {
        ...
        try
        {
            ...
            for (auto const& [topic, index] : bag_topics)
            {
                for (size_t i = 0; i < topic_messages[topic].size(); i++)
                {
                    auto const& [time, message] = topic_messages[topic][i];
                    bag->write(message, topic, topic_names_and_types[topic][0], time);
            ...
            if (!subscribed)
            {
                ...
                if (it != topic_names_and_types.end())
                {
                    ...
                    subscribed = create_generic_subscription(
                        topic, it->second[0], rclcpp::QoS(10),
                        [this, topic](std::shared_ptr<rclcpp::SerializedMessage> msg) { bagger_callback(msg, topic); });
                }
            }
        }
    }

    void add_env_var(std::vector<std::string> const& var)
    {
        for (auto& topic : var)
        {
    ...
    void get_params()
    {
        ...
        for (std::string const& topic : topics_param)
        {
            ...
            if (tokens.size() == 2)
            {
                ...
                {
                    RCLCPP_WARN(get_logger(), "Failed to set stream time %s for topic %s.", tokens[0].c_str(),
                                tokens[1].c_str());
                }
            }
