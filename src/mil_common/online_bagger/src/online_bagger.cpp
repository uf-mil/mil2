...
class TopicList : public ComponentBase
{
  public:
    ...
    std::vector<std::string> const get_selected()
    {
...
class Tui
{
  public:
    ...
    void spin(std::shared_ptr<online_bagger::Client> client)
    {
        ...
        {
            client->get_bag_topics(
                [&](Client::TopicsFuture future)
                {
        ...
        Component ui_status_bar = Renderer(
            [this, client]
            {
                ...
                {
                    return hbox(
                        { text("Connecting to bag server "), spinner(15, clock.now().nanoseconds() / 200'000'000) });
                }
        ...
        Component ui_bottom_bar = Container::Horizontal(
            { ui_status_bar | vcenter | flex, Button("Quit", on_quit, ButtonOption::Border()) | align_right,
              Maybe(Button("Refresh", on_refresh, ButtonOption::Border()),
                    [client] { return client->get_state() != Client::State::Waiting; }) |
                  align_right,
              Maybe(Button("Bag", on_bag, ButtonOption::Border()),
                    [client] { return client->get_state() == Client::State::Ready; }) |
                  align_right });

        // Main UI
        Component main_ui = Container::Vertical({ ui_head, Renderer([] { return separator(); }), ui_body | flex,
                                                  Renderer([] { return separator(); }), ui_bottom_bar });

        last_state = Client::State::Waiting;
        std::thread refresh_thread(
            [this, client, &on_refresh]
            {
                while (!exited)
                {
  ...
  private:
    ...
    bool ask_bag_detail(std::string& name, float& time)
    {
        ...
        Component ui_time_input =
            Input(&time_input, InputOption::Default()) |
            CatchEvent(
                [](Event event)
                {
        ...
        Component content =
            Renderer(Container::Vertical({ ui_name_input, ui_time_input }),
                     [=]
                     {
                         return gridbox({ { text("Bag File Name ") | vcenter, ui_name_input->Render() | border },
...
{
    std::cout << "Usage: online_bagger <command> [args]." << std::endl;
    std::cout << "Available commands are:" << std::endl;
    std::cout << "list:\tList all topics that can be bagged." << std::endl;
    std::cout << "help:\tShow this help message." << std::endl;
    std::cout << "bag:\tStart bagging." << std::endl;
    std::cout << "\t  --topics -p \tTopics to bag." << std::endl;
    std::cout << "\t\t\tWill bag all message if not specified." << std::endl;
    std::cout << "\t  --name -n \tBag file name." << std::endl;
    std::cout << "\t\t\tDefault name is generate from bagging start time in the format HH:MM:SS." << std::endl;
    std::cout << "\t  --time -t \tDuration in seconds to bag from the past up to now." << std::endl;
    std::cout << "\t\t\tDefault value is 1." << std::endl;
    std::cout << "tui:\tShow terminal user interface." << std::endl;
}
...
int main(int argc, char** argv)
{
    ...
    global_opts.add_options()("subcommand", po::value<std::string>(&subcommand), "Subcommand")(
    ...
    po::variables_map vm;
    po::store(po::command_line_parser(argc, argv).options(global_opts).positional(pos).allow_unregistered().run(), vm);
    po::notify(vm);
    ...
    std::unordered_map<std::string, Mode> const subcmd_mapping = {
        { "tui", Mode::TUI }, { "help", Mode::HELP }, { "bag", Mode::BAG }, { "list", Mode::LIST }
    };
    ...
    else if (mode == Mode::LIST)
    {
        auto future = client->get_bag_topics(
            [](Client::TopicsFuture future)
            {
                for (std::string const& topic : future.get())
                {
        ...
        if (rclcpp::spin_until_future_complete(client, future, std::chrono::seconds(2)) ==
    ...
    else if (mode == Mode::BAG)
    {
            ...
            rclcpp::FutureReturnCode::SUCCESS)
        {
            ...
            bag_opts.add_options()("name,n", po::value<std::string>(&bag_options.goal.bag_name),
                                   "Bag file name")("time,t", po::value<float>(&bag_options.goal.bag_time),
                                                    "Duration in seconds to bag from the past up to now ")(
                "topics,p", po::value<std::vector<std::string>>(&bag_options.goal.topics)->multitoken(),
                "Topics to bag");

            po::variables_map bag_vm;
            po::store(po::command_line_parser(options).options(bag_opts).run(), bag_vm);
            po::notify(bag_vm);
