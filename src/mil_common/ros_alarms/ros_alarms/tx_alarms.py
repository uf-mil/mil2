...
from ros2_alarms_msgs.msg import Alarm
from ros2_alarms_msgs.srv import AlarmGet, AlarmGetRequest, AlarmSet, AlarmSetRequest



async def _check_for_alarm(nh: Node, alarm_name: str, nowarn=False):
    if (
        not nowarn
        and (await nh.has_parameter("known_alarms"))
        and alarm_name not in (await nh.get_parameter("known_alarms"))
    ):
