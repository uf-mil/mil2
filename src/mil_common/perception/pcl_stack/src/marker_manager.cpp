...
void MarkerManager::feedbackCb(visualization_msgs::InteractiveMarkerFeedbackConstPtr const& feedback)
{
...
void MarkerManager::update_interactive_marker(mil_msgs::PerceptionObject const& object)
{
    ...
    if (is_new)
        interactive_marker_server_->insert(int_marker,
                                           std::bind(&MarkerManager::feedbackCb, this, std::placeholders::_1));
...
void MarkerManager::update_markers()
{
    ...
    for (auto const& pair : objects_->objects_)
    {
