...
NO_POLES_SURGE = 1.00 # surge when no poles ever seen
...
class NavigateChannelServer(Node):
    def __init__(self) -> None:
        ...
        self._as = ActionServer(
            self, NavigateChannelAction, "navigatechannel",
        ...
        self.create_subscription(
            PerceptionTargetArray, "/perception/targets",
    ...
    async def move_rel(self, surge, yaw):
        ...
        self.get_logger().info(f"Sending move command: surge={surge:.2f}m, yaw={yaw:.3f}rad")
        self.get_logger().debug(f"Full pose: pos=({pose.position.x:.2f}, {pose.position.y:.2f}, {pose.position.z:.2f})")
    ...
    async def inspection(self, inspection_rad, precision = 3):
        ...
        self.get_logger().info(f"Looking right...")
        for _ in range(precision):
            ...
            if success:
                await self._sleep(LOOK_DWELL_SEC)
                if self._small() or self._big():
                    self.get_logger().info(f"Found poles on right side")
                    return True

        # Back to center
        await self.move_rel(0.0, inspection_rad)

        self.get_logger().info(f"No poles found in inspection")
        return False
    
    def _filter(self, label, min_area) -> List:
        return [d for d in self._detections
                if d.label == label and d.width*d.height > min_area]

    def _big(self):
        return self._filter("red-pole", MIN_AREA_CLOSE) + \
               self._filter("white-pole", MIN_AREA_CLOSE)

    def _small(self):
        return self._filter("red-pole", MIN_AREA_DETECT) + \
               self._filter("white-pole", MIN_AREA_DETECT)

    def _log_areas(self):
        if self._detections:
            areas = ", ".join(f"{d.label}:{d.width*d.height:.0f}" for d in self._detections)
    ...
    @staticmethod
    def encroachment_weight(label: str, cx: float, image_w: int = 960,
                            deadband_px: float = 0, overshoot_gain: float = 1.2,
                            overshoot_exp: float = 1.6) -> float:
        cx_mid = image_w * 0.5
        half = image_w * 0.5
    ...
    async def final_inspection(self, inspection_rad, precision=3):
        ...
        self.get_logger().info(f"Starting FINAL inspection (will return to center)")
        
        step = inspection_rad / max(1, precision)
        found_any = False
        
        # Look left
        self.get_logger().info(f"Final inspection: Looking left...")
        for i in range(precision):
            ...
            if success:
                ...
                if (self._small() or self._big()) and not found_any:
                    found_any = True
                    self.get_logger().info(f"Final inspection: Found poles on left")
        
        #return to center from left
        await self.move_rel(0.0, -inspection_rad)
        
        # Look right
        self.get_logger().info(f"Final inspection: Looking right...")
        for i in range(precision):
            success = await self.move_rel(0.0, -step)
            if success:
                await self._sleep(LOOK_DWELL_SEC)
                if (self._small() or self._big()) and not found_any:
                    found_any = True
                    self.get_logger().info(f"Final inspection: Found poles on right")
    ...
    async def _execute(self, gh, absenceSurge = 1.5, imageWidth = 960):
        if not self._move.wait_for_server(timeout_sec=5.0):
            ...
            return NavigateChannelAction.Result(
                success=False, message="move server unavailable"
            )
        ...
        while rclpy.ok():
            # current detections
            real = [d for d in self._detections if d.width * d.height >= MIN_AREA_DETECT]
            ...
            if not self._ever_seen and total == 0:
                self.get_logger().info(f"=== SEARCH PATTERN: No poles ever seen ===")
                ...
                if found:
                    ...
                    self.get_logger().info("No poles found, continuing search pattern...")
                continue
            ...
            if self._ever_seen_steerable and total == 0:
                ...
                self.get_logger().info(f"Poles lost for {elapsed:.1f}s (timeout={LOST_TIMEOUT}s)")

                if elapsed > LOST_TIMEOUT:
                    self.get_logger().info("Lost timeout reached, performing FINAL scan")
                    ...
                    if len([d for d in self._detections if d.width * d.height >= MIN_AREA_DETECT]) > 0:
                        self.get_logger().info("Found poles after moving forward, continuing navigation")
                        self._lost_since = self.get_clock().now()
                        continue
                    ...
                    if found:
                        self.get_logger().info("Poles detected in final scan, pushing through straight")
                        await self.move_rel(1.0, 0.0)
                        self._lost_since = self.get_clock().now()
                    else:
                        # We're done!
                        self.get_logger().info("No poles found in final scan, channel cleared!")
                        gh.succeed()
                        return NavigateChannelAction.Result(
                            success=True, message="channel cleared"
                        )
                else:
            ...
            if len(big) > 0:
                ...
                if max_area > 6000:
                    self.get_logger().warning(f"VERY close to {max_pole.label} (area={max_area:.0f}), steering away")
            ...
            if not self._ever_seen_steerable and (len(steerable) == 0 and len(real) > 0):
                ...
                if weight_sum > 0:
                    ...
                    err_px = avg_cx - CX  # Positive = poles to right, need to turn right
                ...
                self.get_logger().info(f"Approaching {len(real)} small poles: yaw={yaw_cmd:.3f}")
                await self.move_rel(0.0, yaw_cmd)
            ...
            if len(steerable) == 0:
                self.get_logger().info("No poles large enough for steering, moving straight")
            ...
            self.get_logger().info(f"Using largest pole: {largest.label} at cx={largest.cx}, area={max_area:.0f}")

            # Calculate weights
            area = float(largest.width * largest.height)
            w_a = self.area_weight(area, a0=MIN_AREA_STEER, a1=MIN_AREA_CLOSE)
            w_c = self.encroachment_weight(largest.label, largest.cx, image_w=IMAGE_W,
                                        deadband_px=(0),
                                        overshoot_gain=1.0,
                                        overshoot_exp=0.9)
            tau = w_a * w_c
            ...
            self.get_logger().info(f"Steering based on largest {largest.label}: tau={tau:.2f}, yaw={yaw_cmd:.3f}, surge={surge:.2f}")
            await self.move_rel(0.0, yaw_cmd)
        ...
        gh.abort()
        return NavigateChannelAction.Result(
            success=False, message="node shutdown"
        )
