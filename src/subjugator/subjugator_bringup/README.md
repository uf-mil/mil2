...
## Down-cam centering (HoneOverTable) in sim

The `mission_planner` down-cam subscribers default to the **real-robot** topics,
so they must be overridden to run the down-cam centering (the `HoneOverTable`
subtree / `HoneOverTarget` node) in simulation:
...
- **Model:** centering only actually converges once the down YOLO loads a model
  whose class matches `HoneOverTarget`'s `label` (default `table`). Until then the
