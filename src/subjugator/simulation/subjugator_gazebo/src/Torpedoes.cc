                          ...
                          gz::sim::EntityComponentManager &ecm, gz::sim::EventManager &eventMgr)
{
    ...
    this->service_ = this->torpedo_node_->create_service<std_srvs::srv::SetBool>(
        "torpedo_launcher/launch_torpedo",
        [this](std::shared_ptr<std_srvs::srv::SetBool::Request> const request,
               std::shared_ptr<std_srvs::srv::SetBool::Response> response) { this->LaunchTorpedo(request, response); });
...
void Torpedoes::LaunchTorpedo(std::shared_ptr<std_srvs::srv::SetBool::Request> const request,
                              std::shared_ptr<std_srvs::srv::SetBool::Response> response)
{
    ...
    response->message = "Torpedo launch state: " + std::to_string(request->data);
    std::cout << "[Torpedoes] Service call received. Torpedo State: " << request->data << std::endl;
}
...
void Torpedoes::PostUpdate(gz::sim::UpdateInfo const &info, gz::sim::EntityComponentManager const &ecm)
{
    ...
    if (torpedoCount < 2 && (t_pressed || service_called))
    {
