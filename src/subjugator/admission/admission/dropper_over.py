...
def quat_from_odom(odom):
    q = Quaternion()
    (q.w, q.x, q.y, q.z) = transforms3d.taitbryan.euler2quat(yaw_from_odom(odom), 0, 0)
    return q
...
def relative(track, odom):
    ...
    x, y = ROT.rotate(
        np.array(
            [320 - track.bbox.center.position.x, track.bbox.center.position.y - 180],
        )
        * SCALE,
    )
...
def absolute(track, odom):
    ...
    return (x + odom.pose.pose.position.x, y + odom.pose.pose.position.y)
...
def pick_track(yolo, odom, visited, visited_dets):
    ...
    for det in yolo.detections:
        ...
        for vx, vy in visited:
            angle = math.fabs(
                math.remainder(
                    math.atan2(vy - oy, vx - ox) - math.atan2(y - oy, x - ox),
                    math.tau,
                ),
            )
