...
// The documented deadband (|effort| <= 0.07 produces no thrust). The interior is
// exactly 0 N because both bracketing samples are zero; the band edges land
// between a zero and a nonzero sample, so interpolation leaves fp dust there
// rather than a literal 0. Either way a near-zero wrench produces no meaningful
// creep, and the first sample past the band does produce thrust.
TEST(Lut, DeadbandSpansDocumentedRange)
{
    for (double effort = -0.06; effort <= 0.06 + 1e-12; effort += 0.01)
    {
        EXPECT_DOUBLE_EQ(force_from_effort(effort), 0.0) << "expected deadband interior at effort " << effort;
    }
    EXPECT_NEAR(force_from_effort(-0.07), 0.0, 1e-9);
    EXPECT_NEAR(force_from_effort(0.07), 0.0, 1e-9);
    // First samples outside the band produce thrust.
    EXPECT_LT(force_from_effort(-0.08), 0.0);
    EXPECT_GT(force_from_effort(0.08), 0.0);
}

// The asymmetry the issue exists to capture is real and substantial: full
// forward thrust exceeds full reverse by a wide margin (the measured curve is
// ~29% stronger forward), well beyond the ~20% the issue cites. A linear model
// would have this ratio at 1.0.
TEST(Lut, AsymmetryMatchesIssueClaim)
{
    double const forward = force_from_effort(1.0);
    double const reverse = std::abs(force_from_effort(-1.0));
    EXPECT_GT(forward / reverse, 1.20) << "forward/reverse asymmetry weaker than documented";
}
