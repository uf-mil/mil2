...
// correction at a time. Both cameras are closed on with a P controller on the
// pixel error, no lens model needed: yaw until the front camera has it centred,
// creep forward, then sidestep until it is centred in the down camera. Keeps
// creeping forward for `blind_steps` moves when neither camera sees it, then
// gives up. Assumes the down camera's image top is the sub's front and image
// right its right.
class LightFollower final : public BT::DecoratorNode
{
  public:
    ...
    static BT::PortsList providedPorts()
    {
        return {
            ...
            BT::InputPort<double>("kp", 0.7, "Yaw gain on the front-camera pixel error (0-1)"),
            BT::InputPort<double>("max_yaw_deg", 12.0, "Largest yaw correction per step"),
            BT::InputPort<double>("step", 0.5, "Metres per correction"),
            BT::InputPort<double>("tol", 0.1, "Centred when the pixel offset is within this (1 = edge)"),
  ...
  private:
    ...
    BT::NodeStatus tick() override
    {
        ...
        {
            std::string color;
            double kp = 0.7, max_yaw_deg = 12.0, step = 0.5, tol = 0.1;
            int blind_steps = 2;
            ...
            if (ahead)
            {
                double const yaw = std::clamp(-kp * ahead->x * max_yaw_deg, -max_yaw_deg, max_yaw_deg);
                correct(std::abs(ahead->x) < tol ? step : 0.0, 0.0, yaw);
            }
