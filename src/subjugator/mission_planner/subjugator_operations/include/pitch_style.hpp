...
class PitchStyle : public BT::ActionNodeBase
{
  public:
    ...
    BT::NodeStatus tick() override
    {
        ...
        if (state_rn == BT::NodeStatus::RUNNING && gained_momentum == false)
        {
            ...
            // pitch in the OPPOSITE direction to build momentum
            // pitch is controlled by front/back horizontal thrusters
        ...
        else if (state_rn == BT::NodeStatus::RUNNING && gained_momentum == true)
        {
            ...
            // publish strongest pitch command
            auto msg = subjugator_msgs::msg::ThrusterEfforts();
            ...
            // strip roll and pitch, keep only yaw
  ...
  private:
    ...
    void get_port_data()
    {
        ...
        if (!ctx_res)
        {
            throw BT::RuntimeError("PitchStyle requires [ctx] input");
        }
        ctx_ = ctx_res.value();
        ...
        if (!milliseconds_res)
        {
            throw BT::RuntimeError("PitchStyle requires [milliseconds] input");
        }
        ...
        if (!momentum_ms_res)
        {
            throw BT::RuntimeError("PitchStyle requires [momentum_ms] input");
        }
