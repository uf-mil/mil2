  ...
  public:
    JoySubscriber() : Node("joy_subscriber")
    {
        ...
        auto joy_callback = [this](sensor_msgs::msg::Joy::UniquePtr msg) -> void
        {
            ...
            wrench_msg.torque.x =
                (msg->axes[6] == 1) ? 1.0 : ((msg->axes[6] == -1.0) ? -1.0 : 0.0);         // x and b button for roll
            wrench_msg.torque.y = msg->buttons[3] ? 1.0 : (msg->buttons[0] ? -1.0 : 0.0);  // y and a buttons for pitch
                                                                                           // (pointing down or up)
            wrench_msg.torque.z =
                msg->buttons[1] ? 1.0 :
                                  (msg->buttons[2] ? -1.0 : 0.0);  // left and right of directional pad button for yaw

            // RCLCPP_INFO(this->get_logger(), "Force: [%.2f, %.2f, %.2f], Torque: [%.1f, %.1f, %.1f]",
            // wrench_msg.force.x, wrench_msg.force.y, wrench_msg.force.z, wrench_msg.torque.x, wrench_msg.torque.y,
            // wrench_msg.torque.z);
