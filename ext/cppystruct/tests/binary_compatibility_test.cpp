...
template <typename T>
std::string escapeString(T const& val)
{
    std::string out = "\"";
    for (auto c : val)
    {
...
std::string buildPythonScript(Fmt, std::string const& outputPath, Args const&... toPack)
{
...
template <typename Fmt, typename... Args>
auto runPythonPack(Fmt, Args const&... toPack)
{
    ...
    std::vector<char> outputBuffer((std::istreambuf_iterator<char>(inputFile)), (std::istreambuf_iterator<char>()));
...
template <typename Fmt, typename... Args>
void testPackAgainstPython(Fmt, Args const&... toPack)
{
